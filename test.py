...
for iteration in range(0, 10):
	...
	#PROCESSED_TRAINING_DATA = np.reshape(np.dot(training_data, ELASTICITIES), (np.shape(training_data)[0],1))
	#PROCESSED_TEST_DATA = np.reshape(np.dot(test_data, ELASTICITIES), (np.shape(test_data)[0],1))
